...
While the harness is running it publishes its connection info to `/tmp/jobfinder/harness.json` (`url`, `port`, `pid`, and the `screen` / `keys` endpoint URLs) and removes the file on exit, so an agent can discover a running instance without being told the port.
