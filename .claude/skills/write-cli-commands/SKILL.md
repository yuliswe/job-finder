...
description: Load into context when the user asks you to implement or modify a CLI command including its options, arguments, and completion behavior.
---
...
- [ ] **Regenerate `__generated__/cli/_completion.zsh`** by running `./src/cli/bin/cli completion`. The generator walks the commander tree and inlines every option's `argChoices`, so static enums get tab-completion automatically — DO NOT hand-edit `_completion.zsh`.
