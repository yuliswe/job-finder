...
Remember to update the relevant config file(s) under `examples/` (e.g. `jobfinder-ollama.config.js`, `jobfinder-openrouter.config.js`) when modifying or adding new configuration options. Each config file corresponds to a different LLM plugin and may have unique options, but they share some common ones (e.g. `SEEDS_DIR`, `DB_PATH`) that should be kept in sync across files if modified.
