...
- \`console.log\` / \`console.warn\` / \`console.error\` / \`console.info\` output IS captured during validation and surfaced to you in the next feedback message. USE THEM LIBERALLY to explore the page: log the structure you see (\`document.querySelectorAll(...).length\`, an example card's \`outerHTML\`, the JSON shape of an internal API response, etc.) when you're unsure. The captured log is included verbatim in the failure feedback so you can iterate quickly.
