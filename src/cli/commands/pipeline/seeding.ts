...
async function runSeeding(): Promise<void> {
  ...
  const { sourceSeedInserted } = await insertSeeds(jobs);
  terminal.log(`Inserted ${sourceSeedInserted} rows into SourceSeed`);
}
...
async function validateScrapeArgs(
...
async function insertSeeds(
  jobs: JobResult[]
): Promise<{ sourceSeedInserted: number }> {
...
// Seeding has no parent record at iteration start (the SourceSeed doesn't
// exist until we insert it), so it doesn't go through processOne — instead it
// keeps a local try/catch around the insert and silently skips on failure.
async function seedOne(args: {
  ...
  try {
    ...
    // Seeding "completes" the moment the row exists — mark it processed
    // inline so progress reflects reality.
    await markTriggerProcessed({
      task: 'seeding',
      entity: { ofSourceSeedId: id },
    });
    return { sourceSeedInserted: 1 };
  } catch {
    // unique-url collision or other insert error — skip
    return { sourceSeedInserted: 0 };
  }
}
