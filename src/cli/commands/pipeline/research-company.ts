...
import {
  previouslyQueuedForResearchCompany,
  inScopeForResearchCompany,
  neededForResearchCompany,
  qualifiedForResearchCompany,
...
type ResearchCompanyOptions = {
   ...
   * spam the terminal. Individual `jobfinder pipeline research-company` invocations
   * leave it false so the user sees actionable feedback. */
  suppressNothingToDoLog?: boolean;
};

export function createResearchCompanyCommand(): Command {
  return new Command('research-company')
    .description(
    ...
    .action(async (opts: ResearchCompanyOptions) => {
      if (opts.start) {
        await withBrowserInstance(context => runResearchCompany(context, opts));
        return;
      }

      await queueResearchCompany(opts);
    });
}
...
async function pickResearchCompanyTargets(
  opts: ResearchCompanyOptions
  ...
  if (mode === 'all') await requeueAllInScope('research-company');

  // Picker = qualifiedForX ∩ inScopeForX [∩ neededForX] + state filter.
  // `--all` deliberately drops `neededForResearchCompany` so already-complete rows
  ...
  // still apply `neededForResearchCompany` so rows whose output columns are all
  // filled are skipped.
  ...
  //   ∨ (row is currently queued for research-company).
  ...
  const query = db
    ...
    .where(eb =>
      eb.or([
        eb.and([
          qualifiedForResearchCompany(eb),
          inScopeForResearchCompany(eb),
          ...(mode !== 'all' ? [neededForResearchCompany(eb)] : []),
          ...(stateFilter ? [stateFilter(eb)] : []),
        ]),
        previouslyQueuedForResearchCompany(eb),
      ])
    );
...
export async function queueResearchCompany(
  opts: ResearchCompanyOptions
): Promise<{ queued: number }> {
  const mode = pipelineModeFromOptions(opts);
  const targets = await pickResearchCompanyTargets(opts);
  ...
  terminal.log(
    `${targets.length} JobSource row(s) queued for research-company. Pass --start (or \`jobfinder start-pipeline\`) to process them.`
  );
...
export async function runResearchCompany(
  ...
  await reapStaleStartedStates('research-company');
  const sources = await pickResearchCompanyTargets(opts);

  if (sources.length === 0) {
    if (!opts.suppressNothingToDoLog) {
      terminal.log('No JobSource rows need research-company. Nothing to do.');
    }
